...
<div class="sidebar d-flex flex-column">
    ...
    <ul class="nav flex-column mb-auto">
        ...
        <li class="nav-item">
            <a class="nav-link <?= (url_is('*dashboard*')) ? 'active' : '' ?>" href="/<?= $tenantStringId ?? session('current_tenant_string') ?>/instructor/dashboard">
                <i class="fa-solid fa-gauge-high"></i> Dashboard
            </a>
        </li>
        
        <li class="nav-item">
            <a class="nav-link <?= (url_is('*courses*') || url_is('*course/*')) ? 'active' : '' ?>" href="/<?= $tenantStringId ?? session('current_tenant_string') ?>/instructor/courses">
                <i class="fa-solid fa-book"></i> Manajemen Kelas
            </a>
        </li>

        <li class="nav-item">
            <a class="nav-link <?= (url_is('*students*')) ? 'active' : '' ?>" href="/<?= $tenantStringId ?? session('current_tenant_string') ?>/instructor/students">
                <i class="fa-solid fa-users"></i> Manajemen Peserta
            </a>
        </li>
        
        <li class="px-4 mt-3 mb-2 text-uppercase small text-muted fw-bold" style="font-size: 0.7rem;">CBT & Penilaian</li>

        <li class="nav-item">
            <a class="nav-link <?= (url_is('*quiz-bank*')) ? 'active' : '' ?>" href="/<?= $tenantStringId ?? session('current_tenant_string') ?>/instructor/quiz-banks">
                <i class="fa-solid fa-database"></i> Bank Soal CBT
            </a>
        </li>
        
        <li class="nav-item">
            <a class="nav-link <?= (url_is('*scoring*')) ? 'active' : '' ?>" href="/<?= $tenantStringId ?? session('current_tenant_string') ?>/instructor/scoring">
                <i class="fa-solid fa-star-half-stroke"></i> Penilaian Kelas
            </a>
        </li>
        
        <li class="px-4 mt-3 mb-2 text-uppercase small text-muted fw-bold" style="font-size: 0.7rem;">SISTEM</li>
        <li class="nav-item">
            <a class="nav-link <?= (url_is('*announcements*')) ? 'active' : '' ?>" href="/<?= $tenantStringId ?? session('current_tenant_string') ?>/instructor/announcements">
