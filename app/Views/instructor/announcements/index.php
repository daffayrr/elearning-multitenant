...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <button type="button" class="btn btn-navy rounded-pill px-4" data-bs-toggle="modal" data-bs-target="#addAnnouncementModal">
...
<?php if(empty($announcements)): ?>
    <div class="card border-0 shadow-sm rounded-4 text-center p-5">
        ...
        <p class="text-muted mb-0">Informasi dari Admin atau yang Anda bagikan akan muncul di sini.</p>
    </div>
<?php else: ?>
    <div class="row g-4">
        <?php foreach($announcements as $a): ?>
        <div class="col-12">
            <div class="card border-0 shadow-sm rounded-4 h-100 <?= $a->author_id == session('user_id') ? 'border-start border-4 border-primary' : '' ?>">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        ...
                        <?php if($a->author_id == session('user_id')): ?>
                        <form action="/<?= $tenantStringId ?? session('current_tenant_string') ?>/instructor/announcements/<?= $a->id ?>/delete" method="POST">
                            <?= csrf_field() ?>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow">
      <div class="modal-header border-bottom-0 pb-0">
        <h5 class="modal-title fw-bold">Bagikan Informasi ke Siswa</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form action="/<?= $tenantStringId ?? session('current_tenant_string') ?>/instructor/announcements" method="POST">
          <?= csrf_field() ?>
          <div class="modal-body">
              <div class="alert alert-info border-0 shadow-sm small py-2">
                  <i class="fa-solid fa-circle-info me-2"></i> Informasi ini akan dikirimkan ke dashboard seluruh siswa.
              </div>
              <div class="mb-3">
          ...
          <div class="modal-footer border-top-0 pt-0">
            ...
            <button type="submit" class="btn btn-navy px-4"><i class="fa-solid fa-paper-plane me-1"></i> Bagikan</button>
          </div>
      </form>
    </div>
  </div>
